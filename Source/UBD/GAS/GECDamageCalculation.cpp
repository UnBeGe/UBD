...
#include "GECDamageCalculation.h"
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		...
		DEFINE_ATTRIBUTE_CAPTUREDEF(UAdventureAttributeSet, Damage, Source, false);

		DEFINE_ATTRIBUTE_CAPTUREDEF(UAdventureAttributeSet, Armor, Target, false);
		DEFINE_ATTRIBUTE_CAPTUREDEF(UAdventureAttributeSet, Health, Target, false);
	}
};
...
UGECDamageCalculation::UGECDamageCalculation()
{
...
void UGECDamageCalculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	float DamageDone = (BaseDamage) * (1 - (Armor / 100));
